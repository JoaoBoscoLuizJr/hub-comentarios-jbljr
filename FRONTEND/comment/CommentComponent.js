...
class UserSecction extends User{
     _user = new User();
...
const loadComment = () => {
    ...
    CommentService.apiGetComment().then(result => {
        const comments = result.map(
            (comment) => new Comment(comment.id, comment.author, comment.comment_text, comment.created_at,
             comment.updated_at)
        );
    ...
    comments.forEach(item => {
        ...
        divDisplay.innerHTML = `
            ...
            <p class="pb-3 mb-0 small lh-sm text-gray-dark">
                ...
                <span class="date-style badge text-bg-secondary">${formattedDate(item.getCreatedAt())}</span>
                </strong>
                <span class="comment">
                ${item.getComment()}
                </span>
